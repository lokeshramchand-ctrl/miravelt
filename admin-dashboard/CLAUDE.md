...
`BACKEND_URL`/`MIRAVELT_API_KEY`/`MIRAVELT_ADMIN_KEY` to match the backend's own `.env`, plus a fresh
...
- The browser only ever holds an **httpOnly, encrypted session cookie** (`iron-session`). It never
  sees `MIRAVELT_API_KEY`, `MIRAVELT_ADMIN_KEY`, or either JWT directly - client components must never
  fetch the backend directly or hold a credential.
- `src/lib/backend.ts` (`"server-only"`) is the single place that calls miravelt-backend; its
  `adminBackendFetch`/`adminBackendJson` attach `X-Miravelt-API-Key` / `X-Miravelt-Admin-Key` /
