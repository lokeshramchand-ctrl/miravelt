...
/** All server-side calls to miravelt-backend go through here. Nothing in this
 * file - the backend URL, MIRAVELT_API_KEY, MIRAVELT_ADMIN_KEY, or any access
...
async function rawFetch(path: string, init: RequestInit = {}): Promise<Response> {
  ...
  headers.set("X-Miravelt-API-Key", requiredEnv("MIRAVELT_API_KEY"));
...
export async function adminBackendFetch(accessToken: string, path: string, init: RequestInit = {}): Promise<Response> {
  ...
  headers.set("X-Miravelt-Admin-Key", requiredEnv("MIRAVELT_ADMIN_KEY"));
