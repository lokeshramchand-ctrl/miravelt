# Miravelt API — Complete Endpoint Reference
...
**Authentication**: unless marked otherwise, every endpoint requires the header `X-Miravelt-API-Key`, checked against `settings.MIRAVELT_API_KEY` (`.env`) using a constant-time comparison — previously this compared against a hardcoded literal regardless of configuration, fixed, see [16 · Known Issues](../16-known-issues-tech-debt.md).
