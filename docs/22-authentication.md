...
|---|---|---|---|
| API key | Which **application** is calling | `X-Miravelt-API-Key` header, compared against `settings.MIRAVELT_API_KEY` via `secrets.compare_digest` (`core/security.py::validate_api_key`) | `dependencies=[Depends(validate_api_key)]` on every `include_router(...)` call in `app.py` — unchanged from before this feature existed |
...
|---|---|---|
| `JWT_SECRET_KEY` | *(required)* | No default — `core/config.py` fails app startup if missing **or** shorter than 32 characters, the same "fail fast on a bad mandatory secret" posture as `MIRAVELT_API_KEY`/`MONGODB_URI`, extended to also reject a present-but-weak value |
| `JWT_ALGORITHM` | `HS256` | |
...
Both schemes are registered automatically (FastAPI introspects the `Security()`-based dependencies used across the app) and appear together in Swagger's "Authorize" dialog at `/docs`: `APIKeyHeader` (`X-Miravelt-API-Key`) and `JWT` (HTTP Bearer). Authorizing with both lets `/docs` exercise every endpoint, including the ones that require both layers.
