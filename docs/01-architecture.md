...
```mermaid
flowchart LR
    subgraph Container["miravelt-backend container (Dockerfile)"]
    ...
    Client(["API Consumer"]) -- "HTTPS + X-Miravelt-API-Key" --> APP
...
- **Auth — two independent layers**: every router except `/health`, `/live`, `/ready`, and `/metrics` is mounted with `dependencies=[Depends(validate_api_key)]` in `app.py`, unchanged from before. `validate_api_key` (`core/security.py`) checks the `X-Miravelt-API-Key` header against `settings.MIRAVELT_API_KEY` — this authenticates the *calling application* (the one trusted client consuming this backend), not an individual end user.
