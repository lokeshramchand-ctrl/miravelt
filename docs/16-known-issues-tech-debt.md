...
### ✅ IMPLEMENTED — Task queue, scheduler, and response cache (Celery + Redis)
**Files**: `tasks/celery_app.py`, `tasks/pipeline_tasks.py`, `tasks/retraining_tasks.py`, `core/cache.py`, `core/rate_limiter.py`

- **Retraining queue now has an executor.** `feedback/retraining_queue.py::trigger_retraining_if_needed` dispatches `tasks.retraining_tasks.run_baseline_training` to a Celery worker instead of stopping at the old TODO. `tasks/retraining_tasks.py::check_and_trigger_retraining` also runs on `RETRAINING_CHECK_INTERVAL_MINUTES` via beat, as a backstop for a slow trickle of corrections that never triggers the inline `BackgroundTasks` check.
- **`/v1/pipelines/*` batch jobs now have automatic scheduling.** `tasks/pipeline_tasks.py` wraps `behavior/run-all`, `embeddings/sync`, `decay/sweep`, `graph/build`, and `clustering/run` as Celery tasks, scheduled via `tasks/celery_app.py`'s `beat_schedule` (intervals configurable via `core/config.py`'s `PIPELINE_*_INTERVAL_MINUTES`). The manual `POST /v1/pipelines/*` endpoints are unchanged and still work — this is a second entry point into the same engine functions, not a replacement.
...
- Requires `REDIS_URI` set and `celery -A tasks.celery_app worker` / `... beat` actually running (see `docker-compose_local.yaml`'s `celery-worker`/`celery-beat` services, or `docker-compose_production.yaml`'s `velar-celery-worker`/`velar-celery-beat`) — everything above still degrades to "disabled," not a crash, if it isn't.

### Still open

- **Training pipelines use synthetic data.** `training/train.py::load_data()` and `training/finetune.py::load_training_data()` generate random/hardcoded datasets. Their docstrings describe querying MongoDB (`transactions`/`behavior_patterns`/`feedback`) instead — building that real data-assembly pipeline is a scoped feature, not a bug fix. The retraining executor above will now faithfully run this synthetic-data training on schedule, which is arguably worse than not running it at all until this is fixed — don't rely on `RETRAINING_CHECK_INTERVAL_MINUTES`-triggered retraining to actually improve the model until this is addressed.
- **Observability endpoints are stubs.** `routers/observability.py` has no Evidently AI / MLflow integration; this requires standing up that infrastructure, which is a product decision outside the scope of fixing existing code.
- **Caching is not applied everywhere it could help.** `/v1/explain` (the most expensive single endpoint — Milvus search + Ollama generation) and `/v1/resolve` remain uncached; both would need careful key design (the retrieved context, not just the request params, determines correctness) that's its own piece of work.
