...
**Authentication**: unless noted otherwise, every endpoint requires the header `X-Miravelt-API-Key`, checked against `settings.MIRAVELT_API_KEY` (see `core/security.py`). A subset of endpoints — marked **Both** in the table below — additionally require `Authorization: Bearer <access token>` (a JWT obtained from `POST /auth/login` or `/auth/refresh`), resolved to a specific `User` by `core/jwt_auth.py::get_current_user`. The API key authenticates the calling application; the JWT authenticates the end user within it. Full detail in [22 · Authentication](./22-authentication.md).
...
| Status | When | Body |
|---|---|---|
| `401` | Missing `X-Miravelt-API-Key` header | `{"detail": "Missing X-Miravelt-API-Key header"}` |
