...
API_KEY_NAME = "X-Miravelt-API-Key"
api_key_header = APIKeyHeader(name=API_KEY_NAME, auto_error=False)

ADMIN_KEY_NAME = "X-Miravelt-Admin-Key"
...
async def validate_api_key(api_key_header: str = Security(api_key_header)) -> str:
    ...
    the single configured MIRAVELT_API_KEY. This is already O(1) with no
    ...
    if not api_key_header:
        raise HTTPException(
            ...
            detail="Missing X-Miravelt-API-Key header"
        )

    if not secrets.compare_digest(api_key_header, settings.MIRAVELT_API_KEY):
...
async def validate_admin_key(admin_key_header: str = Security(admin_key_header)) -> str:
    ...
    with no per-user scope, so MIRAVELT_API_KEY - shipped inside every client
