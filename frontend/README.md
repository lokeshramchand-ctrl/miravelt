...
A financial analyst in your pocket - a Flutter mobile client for the Auvren backend. Add a Google Pay statement PDF and Auvren turns it into a period-based overview, ranked signals, recurring-payment detection, and a searchable transaction activity feed. See `docs/DESIGN_SPEC.md` for the full design spec and `docs/API_REFERENCE.md` for the backend contract this app is built against.

## Prerequisites

- Flutter SDK matching `environment.sdk` in `pubspec.yaml` (`^3.12.1`).
- A running instance of the Auvren backend (see the repo root `../`) reachable from wherever you run the app.
- An issued `X-Auvren-API-Key` value for that backend (see the backend's own docs for issuance - there is no client-facing signup for this key).
...
|---|---|---|
| `ApiEnvironment.production` (release default) | `https://auvren.deploy.lokeshrc.me/` | `AUVREN_API_BASE_URL` |
| `ApiEnvironment.local` | `http://10.0.2.2:9850/` on the Android emulator, `http://localhost:9850/` elsewhere (matches `docker-compose_local.yaml`'s published port) | `AUVREN_LOCAL_API_BASE_URL` |
...
The one value that's still build-time only, via `--dart-define=AUVREN_API_KEY=...`, is the `X-Auvren-API-Key` header sent on every request - it has no usable default and must always be passed (falls back to a `auvren_test_key_123` placeholder otherwise; see `lib/core/config/app_config.dart`).
...
App icons (`android/app/src/main/res/mipmap-*`, `ios/Runner/Assets.xcassets/AppIcon.appiconset`) are still Flutter's default placeholder - swap in Auvren's actual mark before shipping.
