...
Flutter app (frontend/)   ─┐
                            ├─ X-Velar-API-Key + JWT ──▶  FastAPI monolith (app.py)  ──▶  MongoDB (system of record)
...
| Auth (app-level) | `X-Velar-API-Key` — authenticates the *calling application* | `core/security.py` |
| Auth (user-level) | JWT access/refresh tokens — authenticates the *end user* | `core/jwt_auth.py`, `routers/auth.py` |
| Auth (admin-level) | `X-Velar-Admin-Key` — gates operator-only batch jobs & releases | `core/security.py::validate_admin_key`, `routers/admin.py`, `routers/pipelines.py` |
...
- **Two auth keys, not one.** `VELAR_API_KEY` ships inside every client app binary and is trivially extractable — it authenticates "this is the real app," nothing more. `ADMIN_API_KEY` gates `routers/pipelines.py` and release publishing precisely *because* it must never be reachable by anything holding only the client key. Don't collapse these, and don't let `ADMIN_API_KEY` fall back to `VELAR_API_KEY` (`core/config.py` comment is explicit about this).
- **Every router still requires `X-Velar-API-Key` even when a JWT is also required.** The two checks are independent and both enforced — a valid JWT without the API key is rejected before the handler runs. This is intentional defense-in-depth, not redundancy to remove.
...
Next.js Server Actions / Route Handlers  (admin-dashboard only — BFF layer)
        │  attaches X-Velar-API-Key / X-Velar-Admin-Key / Bearer JWT server-side
        ▼
...
- **Client app secrets (`VELAR_API_KEY`) must never gate operator-only/system-wide endpoints** (batch pipelines, release publishing). Those require `ADMIN_API_KEY` on top.
...
- **Secrets stay server-side.** `VELAR_API_KEY`, `VELAR_ADMIN_KEY`, `JWT_SECRET_KEY`, and any user's access/refresh token never reach a browser or client-side bundle. The admin dashboard's `"server-only"` files are the enforcement point — don't move logic out of them to "simplify" a component.
...
- **New batch/offline pipeline** → its own module (matching `behaviour/`, `clustering/`, `graphs/` shape) exposed via `routers/pipelines.py` with `validate_admin_key`, not auto-scheduled (no scheduler exists — see §3).
...
- Let `VELAR_API_KEY` (or any client-embedded secret) gate an admin/operator-only or system-wide endpoint.
